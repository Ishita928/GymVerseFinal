...
const getWorkoutDuration = async (req, res) => {
  try {
    ...
    // Set the time to 00:00:00 for startOfWeek
    startOfWeek.setHours(0, 0, 0, 0);

    // Create endOfWeek date for Sunday and set time to 23:59:59
    const endOfWeek = new Date(startOfWeek);
    endOfWeek.setDate(startOfWeek.getDate() + 6);
    endOfWeek.setHours(23, 59, 59, 999);

    const response = await Workout.aggregate([
      // Match all workout entries for the given user within the current week (Monday to Sunday)
      {
        $match: {
          userId: authId,
      ...
      // Group by date and sum the calories burned for each date
      {
        $group: {
